...
Execution section adds:
  ...
  Update 2026-08-21: POSITION_SIZE_PCT below raised from 0.05 to 0.10 (the
  fresh-account live sizing decision) and ROUNDTRIP_COST_PCT_PER_SIDE corrected
  from 0.1665% to 0.0809% (confirmed 0.07% actual commission, was a 0.15% guess) -
  the "5%/3.75%/..." ladder description above is the ORIGINAL as-built ladder, not
  the current one (now 10%/7.5%/3%/20%); left as-is since it documents why the
  escalation logic was built the way it was, not what the constants equal today.
"""
import os
import numpy as np
import pandas as pd
...
TRADE_LOG_CSV = os.path.join(HERE, "equity_sim_trades_ultimate_2020_2026.csv")
EQUITY_CSV = os.path.join(HERE, "equity_sim_curve_ultimate_2020_2026.csv")
OPEN_POSITIONS_CSV = os.path.join(HERE, "open_positions_ultimate_2020_2026.csv")
...
TRAIL_MA_WINDOW = 50
HARD_STOP_PCT = 0.08
POSITION_SIZE_PCT = 0.10  # updated 2026-08-21: fresh-account (DD=0) live sizing decision
                          # (addendum "Live decision" section), was 0.05 - see
                          # findings_survivorship_try3.md for the 5%/10%/12.5% sweep this
                          # came from. Old findings docs quoting "5% baseline" numbers were
                          # run before this change and remain correctly labeled at 5%.
...
ROUNDTRIP_COST_PCT_PER_SIDE = 0.000809  # 0.07% commission (user's actual rate, updated 2026-08-21
                                          # from the earlier 0.15% online-retail assumption) + 0.005%
                                          # SET trading fee + 0.001% clearing fee + 7% VAT on the
                                          # commission portion only, per side
