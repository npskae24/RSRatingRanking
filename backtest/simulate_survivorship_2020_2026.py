"""
Survivorship-bias correction, "Try 1" from the addendum's 2026-08-21 live
decision section. Copy of simulate_ultimate_2020_2026.py (same signal/scoring/
sizing/exit logic - see that script's docstring for provenance) with ONE
change: the price/volume/low panel is extended with delisted names pulled
from exclusion_list.txt via download_delisted.py, and any position still open
on a symbol's last valid trading date is force-exited at a conservative
TERMINAL_LOSS_PCT instead of silently freezing (the bug simulate_ultimate's
`px.get(sym, entry_price)` fallback would otherwise cause - see its exit
loop, which just `continue`s past a missing price and never resolves the
position).

Of the 98 names in exclusion_list.txt, yfinance returned data for 78; of
those, only 14 have a real data cutoff well before today (genuinely
delisted/suspended - the other 64 are still trading now, just excluded from
the live ranking universe for unrelated reasons, e.g. REIT/fund tickers not
covered by the SET listed-companies HTML scrape). The remaining 21 tickers
returned NO data at all from yfinance - Yahoo has purged them entirely. That
21-ticker gap is NOT measurable this way; report it, don't pretend it's zero.
So this run corrects for 14/98 (14.3%) of the exclusion list, a partial
correction, not the full survivorship gap.
"""
import os
import numpy as np
import pandas as pd
...
EQUITY_CSV = os.path.join(HERE, "equity_sim_curve_survivorship_2020_2026.csv")
TRADE_LOG_CSV = os.path.join(HERE, "equity_sim_trades_survivorship_2020_2026.csv")

DELISTED_CUTOFF = pd.Timestamp("2026-08-18")  # last_valid_index before this = genuinely gone,
                                               # not just excluded-but-still-trading
TERMINAL_LOSS_PCT = 1.0  # conservative: full write-off when a held position's
                          # ticker vanishes with no exit signal fired first
...
def load_panel():
    ...
    d_close = pd.read_csv(DELISTED_PRICE, index_col=0, parse_dates=True).sort_index()
    d_vol = pd.read_csv(DELISTED_VOLUME, index_col=0, parse_dates=True).sort_index()
    d_low = pd.read_csv(DELISTED_LOW, index_col=0, parse_dates=True).sort_index()
    last_valid = d_close.apply(lambda s: s.last_valid_index())
    delisted_syms = last_valid[last_valid < DELISTED_CUTOFF].index.tolist()
    print(f"Splicing in {len(delisted_syms)} genuinely-delisted tickers: {delisted_syms}")

    close = close.join(d_close[delisted_syms], how="outer")
    vol = vol.join(d_vol[delisted_syms], how="outer")
    low = low.join(d_low[delisted_syms], how="outer")
    close = close.sort_index()
    vol = vol.reindex(index=close.index, columns=close.columns)
    low = low.reindex(index=close.index, columns=close.columns)

    # Only the 14 confirmed-delisted symbols get a forced write-off date - NOT
    # every symbol's last_valid_index, which for any still-trading survivor is
    # just SIM_END (today) and would wrongly write off every open position on
    # the simulation's final day.
    delisted_last_valid = last_valid.loc[delisted_syms]
...
def simulate(close, total_score, entry_signal, regime_up, ma_trail, delisted_last_valid):
    ...
    for d in dates:
        ...
        for sym in list(positions.keys()):
            ...
            if pd.isna(p):
                if is_last_day:
                    # data ran out with the position still open and no price today -
                    # force the write-off using the entry price as basis (full loss)
...
def main():
    ...
    print(f"Panel (with delisted names spliced in): {close.shape}, {close.index.min()} to {close.index.max()}")
    ...
    print(f"\n=== Survivorship-corrected (14/98 exclusion-list names spliced in, full write-off on disappearance) ===")
